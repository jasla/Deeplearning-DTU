...
#    saver = tf.train.Saver() # we use this later to save the model
    saver = tf.train.import_meta_graph('models/model_manual_grad_all_classes_600epoch_1000batch.ckpt.meta')
#    sess = tf.Session()
    with tf.Session() as sess:
        saver.restore(sess, 'models/model_manual_grad_all_classes_600epoch_1000batch.ckpt')
...
#            fetches_train = [train_op, loss, l_out, l_z]
            fetches_train = [train_op, loss, l_out,reg_sparse,reg_term]
...
#            _, batch_loss, train_out, train_z = tuple(res_train)
            _, batch_loss, train_out,batch_sparse,batch_reg = tuple(res_train)
            cur_loss += [batch_loss]
...
#        eval_loss, eval_out, eval_z = tuple(res_valid)
        ...
        if do_plot and (epoch+1) % 100 is 0 and False:
...
#            eval_z = pca.transform(eval_enc)
        ...
        print('Epoch %d. Training Error: %.5f. Validation error: %.5f. Sparse cost %.5f. Weight decay cost: %.5f'
              #%(epoch+1,train_loss[-1],valid_loss[-1],beta*eval_sparse,eval_reg))
              %(epoch+1,train_loss[-1],valid_loss[-1],beta*np.mean(cur_sparse),reg_scale/2*np.mean(cur_reg)))
        if epoch == 0:
            continue
        
        # if you want to plot while training, uncomment the code below
        if do_plot and (epoch+1) % 100 is 0:
            ...
            plt.plot(updates[10:], train_loss[10:], color="black")
            plt.plot(updates[10:], valid_loss[10:], color="grey")
