...
#    modelpath = kwargs.get('modelpath',None) # implement. Figure out how to get current activation functions
    alpha = kwargs.get('alpha',0.003)
    beta = kwargs.get("beta",3)
    p_target = kwargs.get("p_target",0.05)
    tau = kwargs.get('tau',0.001)
...
#            saver.restore(sess,modelpath)
#            print("Model restored.")
#            param_names = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
#            params = tuple(sess.run(param_names))
#            names = [param_name.name[:param_name.name.find("/")] for param_name in param_names if param_name.name.endswith("kernel:0")]
...
#    nn_layers.append(make_hidden(input_pl = tf.placeholder(tf.float32,[None,x_train.shape[1]],'x_pl'),hidden_num = n_units[0],
...
#    print("1")
    ...
    for i in range(len(p_act_enc)):
        reg_sparse += beta * tf.reduce_sum(p_target * tf.log(p_target/p_act_enc[i]) + (1-p_target)*tf.log((1-p_target)/(1-p_act_enc[i])))
